...
server.post("/Tickets", function(req, res, next) {
  const error = validateTicket(req.body);
  if (error) {
    debugger;
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new tickets.
    next();
  }
});
...
function validateTicket(ticket) {
  if (!ticket.title) return "Title is required.";
  if (!ticket.id) return "Ticket ID is required.";
  if (!ticket.project) return "Project is required.";
  return "";
}
